...
const Model = ({title, remove, Cancel}) => {
  ...
  return (
    ...
    <div className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-900 w-[85vw] sm:w-[60vw] md:w-[40vw] lg:w-[28vw] p-4 md:p-6 rounded-md z-50 '>
                <div className='flex justify-end hover:cursor-pointer dark:text-gray-100' onClick={Cancel}><ImCross  size={18}/></div>
                ...
                <div className="flex justify-end gap-3 md:gap-5 hover:cursor-pointer text-xs md:text-base mt-4">
                  <button onClick={Cancel} className="font-semibold text-gray-50 bg-gray-600 p-2 rounded hover:cursor-pointer hover:bg-gray-700">CANCEL</button>
                  <button onClick={remove} className="font-semibold text-gray-100  bg-red-600 p-2 rounded hover:cursor-pointer hover:bg-red-400">REMOVE</button>
                </div>
              </div>
              </>
  )
